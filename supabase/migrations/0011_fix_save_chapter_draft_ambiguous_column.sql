-- Fix: save_chapter_draft's bulk-insert step used `unnest(...) as t(o, b)`,
-- and `b` there collided with the outer plpgsql loop variable `b` (used
-- earlier in the same function to walk the split paragraphs), making
-- Postgres raise "column reference \"b\" is ambiguous" the moment the
-- function's insert step actually ran. Found by supabase/tests/
-- publish_pipeline_test.sql. Renaming the unnest alias columns to something
-- that can't collide with a plpgsql variable fixes it; no other behavior
-- changes.
...
begin
  ...
  -- (unnest alias columns renamed away from `o`/`b` -> `new_ordinal`/
  -- `new_body` so they can never collide with a plpgsql variable in this
  -- function's scope -- that collision is exactly what this migration fixes.)
